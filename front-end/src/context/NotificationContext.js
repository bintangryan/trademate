...
export const NotificationProvider = ({ children }) => {
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch('http://localhost:3110/api/notifications', {
    ...
    const markAsRead = async (notificationId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await fetch(`http://localhost:3110/api/notifications/${notificationId}/read`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` },
            });
    ...
    const markAllAsRead = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await fetch('http://localhost:3110/api/notifications/read-all', {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` },
            });
        } catch (error) {
